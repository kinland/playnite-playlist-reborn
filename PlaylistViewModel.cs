    ...
    public class PlaylistViewModel : ObservableObject, IPlaylistSearchSyncTarget, IPlaylistDragReorderState
    {
        ...
        private sealed class HowLongToBeatGameComparer : IComparer
        {
            ...
            public int Compare(object x, object y)
            {
                ...
                HltbSortKeyBuilder.SortKey keyX = BuildSortKey(x as Game);
                HltbSortKeyBuilder.SortKey keyY = BuildSortKey(y as Game);
                return HltbSortKeyBuilder.CompareSortKeys(keyX, keyY, directionSign);
            }

            private HltbSortKeyBuilder.SortKey BuildSortKey(Game game)
            {
                if (game == null)
                {
                    return new HltbSortKeyBuilder.SortKey(false, long.MaxValue, int.MaxValue);
                }
                ...
                {
                    return new HltbSortKeyBuilder.SortKey(false, long.MaxValue, rankIndex);
                }

                return HltbSortKeyBuilder.BuildSortKey(times, renderSettings, rankIndex);
            }
        }
    }
}
